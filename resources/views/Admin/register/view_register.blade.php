...
@section('title') Admin||View All Register @endsection
@section('content')
                                ...
                                @if (isset($event_details))
                                    @foreach ($event_details as $details)
                                        <tr>
                                            ...
                                            <td><a href="edit_register?id={{ $details->id }}"
                                                    class="btn btn-info mt-1 mb-1"><i
                                                        class="ion-compose"></i>&nbsp;Add/Edit Bank Details</a>&nbsp;<a
                                                    href="edit_details?id={{ $details->id }}"
                                                    class="btn btn-info mt-1 mb-1"><i class="ion-compose"></i>&nbsp;Edit
                                                    User Details</a></td>

                                            <td>
                                                @if ($details->status == '1')
                                                    <span class="badge badge-pill badge-primary">Active</span><br>
                                                    <a href="user_approval?id={{ $details->id }}&status=1"
                                                        class="btn btn-danger btn-sm text-white"><i
                                                            class="fas fa-toggle-on"></i> Inactiv </a>
                                                @else
                                                    <span class="badge badge-pill badge-danger">Inactive</span><br>
                                                    <a href="user_approval?id={{ $details->id }}&status=0"
                                                        class="btn btn-primary btn-sm text-white"><i
                                                            class="fas fa-toggle-on"></i> Activ </a>
                                                @endif
                                            </td>
                                        </tr>
                                    @endforeach
